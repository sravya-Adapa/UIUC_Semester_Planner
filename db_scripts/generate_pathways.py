  ...
  for attempt in range(1, retries + 1):
    try:
      response = client.chat.completions.create(
        model=model,
        messages=[
          {"role": "system", "content": "You are an expert academic analyst. Always return valid JSON only."},
...
def extract_json(response):
  ...
  try:
    ...
    with open("openai_error_response.txt", "w", encoding="utf-8") as f:
      f.write(response)
    raise RuntimeError(
      f"Failed to parse JSON: {e}\n"
      f"Response saved to 'openai_error_response.txt'\n"
...
with open("data/processed/uiuc_courses_final.json", "r", encoding="utf-8") as f:
    all_courses = json.load(f)

# Normalize to list
if isinstance(all_courses, dict):
  flattened = []
  if all(isinstance(v, dict) and all(isinstance(c, dict) for c in v.values()) for v in all_courses.values()):
...
BATCH_SIZE = 20
chunks = [all_courses[i:i + BATCH_SIZE] for i in range(0, len(all_courses), BATCH_SIZE)]

tagged_courses = []

# Try to load existing progress
if os.path.exists("tagged_courses.json"):
    try:
        with open("tagged_courses.json", "r", encoding="utf-8") as f:
...
for idx, chunk in enumerate(chunks, start=1):
    ...
    try:
        print(f"  Batch {idx}/{len(chunks)} ({len(chunk)} courses)...", end=" ", flush=True)
        ...
        # Save progress
        with open("tagged_courses.json", "w", encoding="utf-8") as f:
...
try:
    ...
    with open("career_pathways.json", "w", encoding="utf-8") as f:
    ...
    print("💾 Saved to 'career_pathways.json'\n")
    
except Exception as e:
    print(f" Error generating pathways: {str(e)[:500]}")
    raise

print(" Done! Check 'career_pathways.json' for results.")